...
// Specular reflection (pbrt 8.2.2)
final class SpecularReflection (reflectance: Spectrum, fresnel: Fresnel) extends BxDF {
  // BxDF type
  val bxdfType: BxDFType = BxDFType.Reflection | BxDFType.Specular
  ...
  override def sample(wo: Vector, u1: Double, u2: Double): (Spectrum, Vector, Double) = {
    val wi = Vector(-wo.x, -wo.y, wo.z)
    (fresnel(wo.z) * reflectance / wi.z.abs, wi, 1.0)
  }
